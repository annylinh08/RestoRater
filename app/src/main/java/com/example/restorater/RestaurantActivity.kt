...
class RestaurantActivity : AppCompatActivity() {
    ...
    //adaptor class tp store and bind data
    private var adapter: RestaurantActivity.CommentsAdapter? = null
    //create innner classes for adapter and vi=ewholder
    ...
    FirestoreRecyclerAdapter<Comment, CommentsViewHolder>(options) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //section for querying Firestore for existing comments for the selected restuanrant
        //set RecyclerView to use LinearLayout
        commentsRecyclerView.layoutManager = LinearLayoutManager(this)
        //query Firestore
        val query = db.whereEqualTo("restaurantId", intent.getStringExtra("restaurantId"))

        //pass the query results to adapter for display
